...
using Qinshift.LinqMethods;
using Qinshift.LinqMethods.Entities;
using Qinshift.LinqMethods.Helpers;
using static Qinshift.LinqMethods.Entities.Subject;
...
List<string> studentsFullNamesSql = (from std in Database.Students
                                     select $"{std.FirstName} {std.LastName}").ToList();
firstNamesSql.PrintSimple();
studentsFullNamesSql.PrintSimple();
