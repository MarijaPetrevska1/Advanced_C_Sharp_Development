        ...
        public void DeleteStudentById(int id)
        {
            using (SqlConnection connection = new SqlConnection(_connString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    Console.WriteLine(rowsAffected > 0 ? " Student deleted." : " No student found with that ID.");
                }
            }
        }
        ...
        public void UpdateStudentFirstName(int id, string newFirstName)
        {
            using (SqlConnection connection = new SqlConnection(_connString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@Id", id);
                    int rowsAffected = command.ExecuteNonQuery();
                    Console.WriteLine(rowsAffected > 0 ? " Student updated." : " No student found with that ID.");
                }
            }
        }
    }
